...
import { brandsSelector } from '../../Apis/Getters/brands';
import FileUpload from '../../Apis/Setters/FileUpload';
import { UpdateData } from '../../Apis/Setters/UpdateData';
import { EditBrandContext } from '../../Context/EditBrandContext';

const EditBrand = () => {

    // Using Edit Context Api
    const [id, setId] = useContext(EditBrandContext);
    // Getting Details by Id from Redux State
    const brandDetails = useSelector(state => brandsSelector.selectById(state, id));
    ...
    const edit = (e) => {
        ...
        const response = UpdateData({ url: 'brands/update', cred: credentials });
        response.then(res => {
            window.scrollTo(0, 0);
            if (res.data.status) {
                setSuccessMsg({
