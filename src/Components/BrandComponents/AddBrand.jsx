...
const AddBrand = () => {
    ...
    const fileUpload = (e) => {
        ...
        const response = FileUpload({ file: e.target.files[0], path: 'brands' });
        response.then(res => {
            if (res.data.status) {
                setDetails({
                    ...details,
    ...
    const addBrand = (e) => {
        ...
        const response = UpdateData({ url: 'brands/add', cred: credentials });
        response.then(res => {
            window.scrollTo(0, 0);
            if (res.data.status) {
                setSuccessMsg({
                                ...
                                <div className="card-body p-5">
                                    ...
                                    <div className="row">
                                        <div className="mb-4 col-md-6">
                                            <label htmlFor="form-brands/colorImage" className="form-label">Brand Image</label>
                                            <input type="file" name='colorImage' className="form-control" id="form-brands/colorImage"
                                                onChange={fileUpload} />
                                        </div>
                                        <div className="mb-4 col-md-6">
                                            <label htmlFor="form-brands/name" className="form-label">Brand Name</label>
                                            <input type="text" name='name' className="form-control" id="form-brands/instagram" value={details.name} onChange={handleDetails} required />
                                        </div>
                                        <div className="mb-4">
